...
describe(`UT: ${UploadFileDialogComponent.name}`, () => {
  const enum should {
  ...
  let spectator: Spectator<UploadFileDialogComponent> = null;

  beforeEach(() => (spectator = createComponent()));

  it(should.createInstance, () => {
    expect(spectator).not.toBeNull();
    expect(spectator.component).toBeInstanceOf(UploadFileDialogComponent);
  });
});
